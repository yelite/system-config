...
mapkey("<C-4>", { "n", "t" }, function()
    require("my-config.terminal").toggle_bv()
end)
...
local function bind_rust_lsp_keys(bufnr)
    ...
    mapkey("<leader>ix", "n", "<cmd>RustRunnables<cr>", opt, "Rust Runnables")
    ...
    mapkey("<leader>ik", "n", "<cmd>RustHoverAction<cr>", opt, "Hover Action")
...
function M.bind_lsp_keys(client_name, bufnr)
    ...
    mapkey("gra", { "n", "x" }, my_util.code_action, opts, "Code Actions")
    mapkey("grR", "n", "<cmd>Lspsaga finder<cr>", opts, "Lsp Finder")
    mapkey("grr", "n", "<cmd>Telescope lsp_references<cr>", opts, "References")
    mapkey("gri", "n", "<cmd>Telescope lsp_implementations<cr>", opts, "Implementations")
    mapkey("grn", "n", "<cmd>Lspsaga rename<cr>", opts, "Rename")
