{
  ...
  monitorCfg = config.myConfig.desktop.monitors;
in {
    ...
    in
      {
        ...
        "${mod}+Control+Shift+q" = ''exec --no-startup-id "sleep 0.8; xset dpms force off"'';
      ...
      // lib.optionalAttrs (monitorCfg.primary != null) {
        "${mod}+Control+Shift+q" = ''exec --no-startup-id "toggle-display-source ${monitorCfg.primary.id}"'';
      }
      // lib.optionalAttrs (monitorCfg.secondary != null) {
        "${mod}+Control+Shift+w" = ''exec --no-startup-id "toggle-display-source ${monitorCfg.secondary.id}"'';
      }
      // lib.optionalAttrs (monitorCfg.primary != null && monitorCfg.secondary != null) {
        "${mod}+Control+Shift+e" = ''exec --no-startup-id "toggle-display-source ${monitorCfg.primary.id} ${monitorCfg.secondary.id}"'';
      };
