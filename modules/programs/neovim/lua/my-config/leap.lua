-- A fork of https://github.com/ggandor/leap-spooky.nvim/blob/main/lua/leap-spooky.lua
-- This has different key binds and supports treesitter-textobjects
...
local function setup_remote(kwargs)
    ...
    local default_register = (
        vim.o.clipboard == "unnamed" and "*"
        or vim.o.clipboard:match("unnamedplus") and "+"
        or '"'
    )
        ...
        for scope, key in pairs(scopes) do
            for _, textobj in ipairs(text_objects) do
                table.insert(mappings, {
                    scope = scope,
                    keeppos = keeppos,
                    -- This differs from the original version, e.g. "raw" vs "arw"
                    lhs = key .. textobj,
                    action = function()
            ...
            if mode == "o" or not mapping.keeppos then
                vim.keymap.set(mode, mapping.lhs, function()
                    ...
                    local yank_paste = (
                        ...
                        and vim.v.register == default_register
                    )
                    require("leap").leap({
...
local function setup(kwargs)
    require("flit").setup(kwargs.flit or {})
    setup_remote(kwargs.remote or {})
end
